...
class Doc extends Component {
  ...
  updatePeerValue = val => {
    ...
    const updateAutomerge = (baseDoc, message, opFn) => {
      return Automerge.change(baseDoc,
        message,
        doc => {
          opFn(doc)
        })
    ...
    patches.forEach(patch => {
      ...
      patch.diffs.forEach(([operation, changeText]) => {
        switch (operation) {
          case 1: // Insertion
            historyMessage = `${username} added some text (${changeText}) - ${stamp(new Date(Date.now()))}`;
          ...
          case 0: // No Change
            ...
            break;
          case -1: // Deletion
            historyMessage = `${username} removed some text (${changeText}) - ${stamp(new Date(Date.now()))}`;
            newDoc = updateAutomerge(this.doc, historyMessage, (doc) => {
        ...
        if (newDoc) {
          ...
          this.setState({
            ...
            comm.writeMessage({ peerValue: changes, username, diff: changes[0].message });
          });
        }
      });
    });
    ...
    return (
      <>
        ...
        <aside className="App-history">
          History:
          <ul>
            {[...this.state.localHistory].reverse().map((h,i) => <li key={`history-${i}`}>{h}</li>)}
          </ul>
        </aside>
      </>
    )
